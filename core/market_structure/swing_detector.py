...
class SwingDetector:
    ...
    def _create_swing_point(
        ...
        # Step 1 — Previous Swing Distance
        previous_swing = self.state.last_swing
        distance_from_previous = 0.0
        if previous_swing is not None:
            distance_from_previous = abs(
                price - previous_swing.price
            )
        ...
        if atr is not None and atr > 0.0:
            atr_multiple = (
                distance_from_previous / atr
            )

        # Step 3 — Pivot Dominance
        window_pivot_index = self.config.pivot_left
        if swing_type is SwingType.HIGH:
            pivot_dominance = max(
                0.0,
                pivot.high
        ...
        # Step 4 — Confirmation Strength
        confirmation_strength = 1.0

        # Step 5 — Build SwingPoint
        return SwingPoint(
    ...
    @property
    def confirmed_swing_count(
        self,
    ) -> int:
        """
        Return the number of confirmed swings.
        """

        return len(self.state.confirmed_swings)
