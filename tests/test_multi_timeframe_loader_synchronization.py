...
class _FakeHistoryLoader:
    ...
    def load_history(self, symbol: str, timeframe: int, bars: int) -> list[MarketBar]:
...
def test_synchronize_at_excludes_unclosed_higher_timeframe_bars() -> None:
    ...
    assert context.m15_bars[-1].timestamp == observation
    assert context.m5_bars[-1].timestamp == datetime(2026, 1, 1, 10, 55, tzinfo=UTC)
...
def test_load_excludes_current_still_forming_broker_candles() -> None:
    histories = _histories()
    fake = _FakeHistoryLoader(histories)
    loader = MultiTimeframeLoader(
        loader=fake,
        clock=lambda: datetime(2026, 1, 1, 11, 7, tzinfo=UTC),
    )
...
def test_iter_synchronized_yields_only_warmup_complete_snapshots() -> None:
    ...
    for snapshot in snapshots:
        boundary = snapshot.current_bar.timestamp + timedelta(minutes=15)
...
def test_observation_must_match_existing_m15_open() -> None:
    loader = MultiTimeframeLoader(loader=_FakeHistoryLoader(_histories()))

    with pytest.raises(ValueError, match="existing M15"):
...
def test_naive_clock_and_observation_fail_closed() -> None:
    ...
    with pytest.raises(ValueError, match="observation_timestamp"):
        loader.synchronize_at(
            histories=_histories(),
            observation_timestamp=datetime(2026, 1, 1, 10, 45),
        )
