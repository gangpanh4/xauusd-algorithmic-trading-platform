# Market Regime Detection Specification v1.0
...
**Bull trend**: The N-day exponential moving average slope is significantly positive (e.g., > +k σ of the slope distribution), the Hurst exponent H > 0.55 over a rolling window, and realized volatility is below its 70th percentile. Momentum factor (12-1 month return) is in the top quartile of its rolling distribution.

**Bear trend**: Same criteria as bull trend but slope significantly negative, and realized volatility is elevated (often above 60th percentile). Correlation of returns across assets may be rising. Put/call ratio and credit spreads may be included as corroborating signals.
...
### Adopt
- Separate Trend and Volatility dimensions.
- Include an UNKNOWN regime.
- Define regimes using measurable indicators.
- Support multi-timeframe expansion in the future.

### Simplify for v2.3
- Use 4 primary regimes:
  - TREND
  - RANGE
  - VOLATILE
  - UNKNOWN
- Use ADX, ATR, EMA Slope, and BB Width only.
- Use confidence score instead of full probability distributions.
...
Sparse or irregular data (e.g., thinly traded instruments, pre-market bars, early close days) should be detected by comparing the number of valid observations within a rolling window to a minimum observation count. If fewer than M% of expected observations are present in the lookback window, the feature is marked unreliable.
...
### Adopt
- Use OHLCV as the primary market data source.
- Compute regime from derived technical indicators rather than raw prices.
- Require a warm-up period before emitting regime classifications.
- Support multiple timeframes (H4 → H1 → M15).
- Detect stale or incomplete market data.
- Return UNKNOWN when data quality is insufficient.
- Design the detector to accept a symbol parameter for future extensibility.

### Simplify for v2.3
- Single symbol (XAUUSD) only.
- Ignore cross-asset relationships.
- Ignore macroeconomic inputs.
- Ignore order book and open interest.
- Use a fixed 300-candle history requirement.
...
Design Principles
- Separate Trend Strength (ADX) from Trend Direction (EMA Slope)
- Keep feature groups modular
- Design for future extensibility

### Simplify

- Use raw indicator values initially
- Use fixed thresholds
- Single-symbol (XAUUSD) optimization
...
Trend defaults:
ParameterDefaultRationaletrend_entry_thresholdP60 of normalized ADXHistorically, ADX above this level shows meaningful trend persistencetrend_exit_thresholdP45 of normalized ADXCreates a hysteresis band; avoids flip-flopping near boundarytrend_calibration_window_days504Two full years; captures at least one cycletrend_min_dwell_periods3 daysSuppresses single-day signal spikes
Volatility defaults:
...
- Store all thresholds in configuration files.
- Implement hysteresis (separate entry and exit thresholds).
...
- Use absolute thresholds instead of rolling percentiles.
- Manual threshold tuning using backtests.
- Single-symbol optimization (XAUUSD only).
...
- GARCH / EGARCH models
- KS-Test and CUSUM drift detection
- Automatic threshold optimization
- Dynamic threshold drift limits
...
Condition 5 — No active suppression flag. Certain system states suppress transitions outright: a structural break flag raised by the CUSUM monitor, an active data quality warning on the input feed, or a calendar event lock (e.g., within one session of a major scheduled release where artificial signal spikes are expected). During suppression, the current regime label is frozen and the candidate transition is queued for re-evaluation once suppression lifts.
...
### Adopt
- Multi-condition regime transition gates.
- Hysteresis using separate entry and exit thresholds.
- Consecutive confirmation before regime changes.
- Minimum regime duration to reduce noise.
- Data quality validation before transitions.
- Explicit handling of conflicting signals.
- UNKNOWN state when evidence is insufficient.

### Simplify for v2.3
- Use four transition gates:
  - Primary indicators
  - Confidence threshold
  - Confirmation bars
  - Data quality check
- Use fixed confirmation periods:
  - Trend = 3 bars
  - Range = 3 bars
  - Volatile = 2 bars
- Implement basic oscillation logging only.
...
- Crisis-specific asymmetric hysteresis.
- Adaptive confirmation counts.
...
- Increase confidence when multiple independent indicators agree.
- Reduce confidence near decision thresholds.
- Reduce confidence when indicators conflict.
- Return UNKNOWN when confidence falls below a configurable minimum.

### Simplify for v2.3
- Output a single confidence score instead of separate instantaneous and regime confidence.
- Base confidence on:
  - Indicator agreement
  - Threshold distance
  - Regime persistence
  - Data quality
  - Transition status
- Use simple additive scoring instead of Bayesian probability models.
...
- Precision-weighted indicator voting
- Calibration aging adjustments
...
The top-level regime label. Enumeration values must be defined exhaustively in the module schema with no open string values permitted. Recommended values: STRONGLY_TRENDING_UP, STRONGLY_TRENDING_DOWN, WEAKLY_TRENDING_UP, WEAKLY_TRENDING_DOWN, RANGING, HIGH_VOLATILITY, LOW_VOLATILITY, CRISIS, TRANSITIONING, UNKNOWN. UNKNOWN is reserved for initialization and total signal failure states; TRANSITIONING for periods where a regime change is confirmed but the destination regime has not yet met its own confidence floor.
